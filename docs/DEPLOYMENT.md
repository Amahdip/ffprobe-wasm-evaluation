...
- **ffmpeg** available during the Netlify build (for fixture generation)

Netlify’s default build image includes ffmpeg. If a build fails on fixture generation, add a `build` plugin or pre-build step that installs ffmpeg.
...
1. `fixtures:generate` — synthetic sample videos → `public/fixtures/`
2. `vite build` — app + fixtures → `dist/`

Core fixtures (~5–15 MiB) are included in every deploy. Optional heavy fixtures (4K, 10-minute) are **not** generated by default.
